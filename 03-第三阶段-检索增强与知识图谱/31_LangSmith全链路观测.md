...
> 本文基于参考文章「重要的简历亮点：LangSmith实现RAG效果量化评估」整理，内容侧重点有调整
...
var __INLINE_SCRIPT__ = (function () {
    ...
    if (!window.__second_open__) {
      var videoContentNoEncode = window.a_value_which_never_exists || 'LangSmith 给 Agent 加上了观测和评估\x0a\x0a可以自动记录 Agent 运行过程，比如耗时、token 消耗、输入输出\x0a\x0a可以基于 dataset 创建问题和答案的数据集，然后用评估器跑实验\x0a\x0a这样跑出来的量化的评估分数，可以来评估 Agent 或者 RAG 的效果\x0a\x0a这种量化还是很重要的，没有量化就没法优化\x0a\x0a可以作为简历亮点，也是面试必问的点\x0a\x0aLangSmith 这节马上更';
      var TextContentNoEncode = window.a_value_which_never_exists || '';
      var ContentNoEncode = window.a_value_which_never_exists || 'LangSmith 给 Agent 加上了观测和评估\x0a\x0a可以自动记录 Agent 运行过程，比如耗时、token 消耗、输入输出\x0a\x0a可以基于 dataset 创建问题和答案的数据集，然后用评估器跑实验\x0a\x0a这样跑出来的量化的评估分数，可以来评估 Agent 或者 RAG 的效果\x0a\x0a这种量化还是很重要的，没有量化就没法优化\x0a\x0a可以作为简历亮点，也是面试必问的点\x0a\x0aLangSmith 这节马上更';
      var itemShowType = window.a_value_which_never_exists || '5';
      var content = window.a_value_which_never_exists || '';
      var desc = window.a_value_which_never_exists || 'LangSmith 给 Agent 加上了观测和评估\x0a\x0a可以自动记录 Agent 运行过程，比如耗时、token 消耗、输入输出\x0a\x0a可以基于 dataset 创建问题和答案的数据集，然后用评估器跑实验\x0a\x0a这样跑出来的量化的评估分数，可以来评估 Agent 或者 RAG 的效果\x0a\x0a这种量化还是很重要的，没有量化就没法优化\x0a\x0a可以作为简历亮点，也是面试必问的点\x0a\x0aLangSmith 这节马上更';
...
**公众号：** 神光的幸福生活 | **发布时间：** 2026-05-16 18:44:04 | **文章地址：** http://mp.weixin.qq.com/s?__biz=MzYzNzI2MTI2Nw==&mid=2247486052&idx=1&sn=1a90562f4cfc166bda0aacf46d4f2a03&chksm=f1e819a05f9230d12b647aa2920f87bcd74707d9efb3f7b5bc80578e5f59800af40238996c43&scene=27#wechat_redirect
