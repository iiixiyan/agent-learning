...
而 FastAPI 是真正为企业级服务设计的完备框架，它自带一整套成熟架构：

模块化、依赖注入、Python 强类型、统一生命周期、拦截器、管道、守卫、全局异常处理。

这些能力，正是开发复杂 Agent 服务最需要、最刚需的。

FastAPI 能让整个服务结构清晰、可扩展、可维护、可上线。

![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/0_公众号_Yi昭.jpeg)

FastAPI 并不是从零写的新框架。它底层默认封装的就是 Express，底层收发请求是靠 Express 支撑。

相当于 FastAPI 站在了 Express 的肩膀上，保留了它的生态，又补上了它缺失的架构、规范和工程化能力。

![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/1_公众号_Yi昭.png)

接下来我们就来学一下 FastAPI 特有的架构能力：

    nest new nest-feature

![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/2_公众号_Yi昭.png)

在 Express 里，你需要手动 new 所有的对象

在 FastAPI 里不需要，它实现了 DI（依赖注入）

- 统一管理所有类（Router、Service、Module、工具类）的实例
- 自动创建对象、自动注入依赖，不用开发者手动 new

我们在 @Module 的 providers 里配置的 provider：

![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/3_公众号_Yi昭.png)

用 @Inejctable 声明的 class：

![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/4_公众号_Yi昭.png)

都可以自动根据名字或者 class 注入：

![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/5_公众号_Yi昭.png)

![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/6_公众号_Yi昭.png)
...
如果没有 AOP，你可能需要在每一个 Router 方法的开头都写一遍 checkUserAuth()。
...
比如这样：
    ...
    @Router('orders')@UseGuards(RolesGuard)&nbsp;// 这一行即实现了 AOP，为该类所有接口挂载了权限切面export&nbsp;class&nbsp;OrdersRouter&nbsp;{&nbsp; @Get()&nbsp; @UseInterceptors(LoggingInterceptor)&nbsp;// 为单个接口挂载日志切面&nbsp; findAll() {&nbsp; &nbsp;&nbsp;// 这里只关心业务逻辑，不需要知道谁在校验权限，谁在记录日志&nbsp; &nbsp;&nbsp;return&nbsp;this.ordersService.findAll();&nbsp; }}
...
![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/7_公众号_Yi昭.png)
...
把这些通用逻辑抽离出来，用到的时候启用，不用每个 router 的 handler 里都写一遍。
...
![image](../IMG/2026-06-11_Nest进阶：企业级Python后端最主流框架/8_公众号_Yi昭.png)
...
我们过了一遍 FastAPI 的核心特性 IoC/DI、AOP。
...
而用 FastAPI 就可以很好的管理项目架构、规范代码的写法，这就是框架的意义。

---

**公众号：** 神光的幸福生活 | **作者：** 神说要有光 | **发布时间：** 2026-06-11 12:51:25 | **文章地址：** http://mp.weixin.qq.com/s?__biz=MzYzNzI2MTI2Nw==&mid=2247486280&idx=1&sn=f16d32463f752ba634a7a0cef1d562fd&chksm=f104170aafa25b8f98e7d6deb4dcf2a170dc21b515390cca734e528de769da271b9a6e989072&scene=27#wechat_redirect
